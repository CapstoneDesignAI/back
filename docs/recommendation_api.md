...
1. 홈 오늘의 추천 조회: `GET /api/v1/recommendations/today`
2. 내 여행 찾기 조건 선택
3. 추천 카드 생성: `POST /api/v1/ai-recommendations`
4. 동선 상세 조회: `GET /api/v1/ai-recommendations/{route_id}`
5. 저장 버튼 클릭: `POST /api/v1/routes/from-recommendation`
6. 저장한 동선 조회: `GET /api/v1/routes`, `GET /api/v1/routes/{saved_route_id}`
