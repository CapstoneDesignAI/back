...
def test_recommendation_uses_place_name_keyword_for_missing_images(monkeypatch) -> None:
    captured_keywords: list[str] = []

    def fake_fetch_tour_photo_image_url(keyword: str) -> str | None:
        captured_keywords.append(keyword)
        return f"https://example.com/{keyword}.jpg"

    monkeypatch.setattr(
        "app.services.recommendations.fetch_tour_photo_image_url",
        fake_fetch_tour_photo_image_url,
    )
    ...
    assert captured_keywords[:4] == [
        "도담삼봉",
        "단양구경시장",
        "카페산",
        "만천하스카이워크",
    ]
    assert [place.image_url for place in response.places] == [
        "https://example.com/도담삼봉.jpg",
        "https://example.com/단양구경시장.jpg",
        "https://example.com/카페산.jpg",
        "https://example.com/만천하스카이워크.jpg",
    ]
    assert response.card.thumbnail_url == "https://example.com/도담삼봉.jpg"
    assert [place.image_url for place in response.card.place_preview] == [
        "https://example.com/도담삼봉.jpg",
        "https://example.com/단양구경시장.jpg",
        "https://example.com/카페산.jpg",
    ]
